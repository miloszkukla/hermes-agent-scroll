...
export function writeMockProviderConfig(
  ...
  // Title generation uses the primary model, so it would otherwise issue an
  // extra request to the scripted mock after every turn. Tests that configure
  // their own auxiliary section keep full control of that setting.
  const autoTitleDefault = extraConfig?.includes('auxiliary:')
    ? ''
    : 'auxiliary:\n  title_generation:\n    enabled: false\n'

  // Scripted E2E tool calls must execute without an interactive approval card.
...
export async function launchDesktop(
  ...
  const ozoneArgs = env.WAYLAND_DISPLAY && !env.DISPLAY ? ['--ozone-platform=wayland'] : []
...
export interface MockBackendOptions {
  ...
  modelContextLength?: number
  /** Override the approvals mode while exercising read-only tool calls. */
  approvalMode?: 'manual' | 'smart' | 'off'
  /** Extra test-only environment for the spawned desktop/backend pair. */
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const app = await _electron.launch({
    ...
    args: ['--disable-gpu', '--no-sandbox', ...(env.WAYLAND_DISPLAY && !env.DISPLAY ? ['--ozone-platform=wayland'] : [])],
    env,
  })
